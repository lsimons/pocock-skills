...
- `sbp-architecture-review`: review system architecture for mission-critical concerns such as SPOFs, blast radius, and rollback (instead of `/improve-codebase-architecture` / `/codebase-design` when the concern is operational risk, not module depth)
...
- `sbp-secure-code-review`: security review for mission-critical systems, framed by blast radius and customer impact (instead of `/code-review` whenever the review is security-sensitive, covering auth, secrets, input handling, anything customer-facing; run both when in doubt)
